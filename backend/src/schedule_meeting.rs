...
fn schedule_meeting(
    ...
    let participant: crate::dyte::DyteResponse<crate::dyte::DyteAddParticipant> = crate::dyte::add_participant(&meeting.data.id, &preset, name, username)?;
    ...
    let uuid: String = crate::uuid::gen_uuid_with_xorshift(0.1);

    let mut attendees_list = Vec::<String>::new();
    let organizer_email = user.email.to_string(); 
    ...
    for attendee in attendees.split(',') {
        ...
        if !attendee.is_empty() {
            if !attendee.contains('@') {
                ...
                ft_sdk::println!("Invalid Email Address")
            } else {
    ...
    if attendees_list.is_empty() && !attendees.trim().is_empty() {
        // Return an error here
        ft_sdk::println!("Now Email Address input")
   }

   let ics_file = return_ics_file(
        &title, 
        &meeting_page_url, 
        attendees_list.clone(),
        uuid,
...
fn return_ics_file(meeting_title: &str, meeting_url: &str, attendees_list: Vec<String>, uid: String, start_datetime: String, end_datetime: String, organizer_email: String) -> String {
